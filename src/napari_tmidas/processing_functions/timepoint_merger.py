...
@BatchProcessingRegistry.register(
    name="Merge Timepoint Folder",
    suffix="_timeseries",
    description="Merge a folder of timepoint images into a single time series stack (TZYX or TYX). IMPORTANT: Set thread count to 1!",
    parameters={
        "sort_method": {
            "type": str,
            "default": "natural",
            "description": "Sorting method: 'natural' (smart number sorting) or 'alphabetical'",
        },
        "validate_consistency": {
            "type": bool,
            "default": True,
            "description": "Validate that all images have consistent dimensions and data types",
        },
    },
)
def merge_timepoint_folder(
    image: np.ndarray,
    sort_method: str = "natural",
    validate_consistency: bool = True,
) -> np.ndarray:
    """
    Merge a folder of timepoint images into a single time series stack.

    This function takes the folder that was selected in the batch processing widget
    and merges all timepoint images into a single time series array. The input images
    can be either 2D (YX) or 3D (ZYX), and the output will be 3D (TYX) or 4D (TZYX)
    respectively.

    The function automatically uses the same file suffix that was specified in the
    batch processing widget, so no need to specify file extensions again.

    IMPORTANT: This function should be run with thread count = 1 in the batch processing
    widget, as it processes entire folders at once. It will only process the folder
    ...
    sort_method : str
        Method for sorting files: 'natural' for smart number sorting or 'alphabetical'
    validate_consistency : bool
        Whether to validate that all images have consistent dimensions and data types

    Returns:
    --------
    numpy.ndarray
        Time series array with shape (T, Y, X) for 2D input or (T, Z, Y, X) for 3D input,
        OR the original image if folder was already processed
    """
    global _processed_folders

    # Get the folder path and file suffix from the batch processing context
    import inspect
    ...
    # Walk up the call stack to find the ProcessingWorker
    for frame_info in inspect.stack():
        frame_locals = frame_info.frame.f_locals

        # Look for the filepath in the process_file method
        if "filepath" in frame_locals:
            current_file = frame_locals["filepath"]

        # Look for the ProcessingWorker instance to get output folder and suffix
    ...
    if current_file is None:
        raise ValueError(
            "Could not determine current file path from batch processing context"
        )
    ...
    if folder_path in _processed_folders:
        print(f"Folder {folder_name} already processed, skipping this file...")
        return image  # Return original image unchanged

    # Mark this folder as being processed
    _processed_folders.add(folder_path)

    print(f"🔄 PROCESSING FOLDER: {folder_name}")

    # Use the same suffix that was specified in the batch processing widget
    extensions = (
        [input_suffix]
        if input_suffix
        else [".tif", ".tiff", ".png", ".jpg", ".jpeg"]
    )

    print(f"Looking for files with suffix: {input_suffix}")

    # Find ALL timepoint images in the folder (not just the current file)
    try:
        ...
        if len(image_files) < 2:
            print(
                f"Warning: Only found {len(image_files)} images, need at least 2 for time series"
            )
            # Remove from processed set to allow retry
            _processed_folders.discard(folder_path)
            return image

    except Exception as e:
        # Remove from processed set to allow retry
        _processed_folders.discard(folder_path)
        raise ValueError(f"Error finding images in folder: {str(e)}") from e
    ...
    if sort_method == "natural":
        ...
        print(
            f"Warning: Unknown sort method '{sort_method}', using natural sorting"
        )
        image_files.sort(key=lambda x: natural_sort_key(os.path.basename(x)))

    print(f"📁 File order for {len(image_files)} timepoints:")
    ...
    if len(image_files) > 25:
        for i, file_path in enumerate(image_files[-5:], len(image_files) - 5):
            print(f"  {i:2d}: {os.path.basename(file_path)}")

    # Load and merge images
    try:
        time_series, dimension_order = load_and_validate_images(image_files)

        print(
            f"✅ Created time series: {time_series.shape} ({dimension_order})"
        )
        print(f"Data type: {time_series.dtype}")
        print(f"Memory usage: {time_series.nbytes / (1024**2):.1f} MB")

        # Validate the result
        if validate_consistency:
            if np.all(time_series == 0):
                print("⚠️  Warning: Time series contains only zeros")
            elif len(image_files) > 1 and np.all(
                time_series == time_series[0]
            ):
                print("⚠️  Warning: All timepoints appear to be identical")
            else:
                print("✅ Time series validation passed")

        # Save the time series to the output folder
        output_filename = f"{folder_name}_timeseries.tif"
        output_path = os.path.join(output_folder, output_filename)

        print(f"💾 Saving time series to: {output_path}")

        # Calculate file size and determine if we need BigTIFF
        size_gb = time_series.nbytes / (1024**3)
        use_bigtiff = size_gb > 2.0

        if use_bigtiff:
            print(f"📊 Large file ({size_gb:.2f} GB) - using BigTIFF format")
        ...
        print("✅ Successfully saved merged time series!")
        print(f"📁 Output file: {output_filename}")
        print(f"📊 File size: {size_gb:.2f} GB")
        print(f"📐 Final shape: {time_series.shape} ({dimension_order})")
        ...
        raise ValueError(f"Error creating time series: {str(e)}") from e


# Advanced version with more options
@BatchProcessingRegistry.register(
    name="Merge Timepoint Folder (Advanced)",
    suffix="_timeseries_advanced",
...
def merge_timepoint_folder_advanced(
    ...
    try:
        ...
        output_filename = f"{folder_name}_timeseries_advanced.tif"
        output_path = os.path.join(output_folder, output_filename)
